...
const loginUser = asyncHandler(async (req, res) => {
    ...
    if (otp) {
        if (!email)
            throw new ApiError(400, "Email is required for OTP verification");
...
export const googleCallbackLogin = asyncHandler(async (req, res) => {
    ...
    if (!user) {
        ...
        await emailService.sendWelcomeEmail(googleUser.email, googleUser.name);
    }
    const { refreshToken, accessToken } = await generateAccessAndRefreshToken(user._id.toString());
    res
        .cookie("accessToken", accessToken, options)
...
const updateProfile = asyncHandler(async (req, res) => {
    ...
    if (email && /\S+@\S+\.\S+/.test(email)) {
        ...
        if (!isVerified)
            throw new ApiError(400, "Email not verified by OTP");
        updates.email = email;
...
export const applyWholesaler = asyncHandler(async (req, res) => {
    ...
    if (pendingApp) {
        return res
            .status(400)
            .json(new ApiResponse(200, {}, "You already have pending Application"));
    }
    ...
    if (declinedApp) {
        ...
        if (declinedApp.reviewedAt &&
            declinedApp.reviewedAt > threeDaysAgo) {
            const daysLeft = Math.ceil((declinedApp.reviewedAt.getTime() +
                3 * 24 * 60 * 60 * 1000 -
                Date.now()) /
                (24 * 60 * 60 * 1000));
            return res
                .status(400)
                .json(new ApiResponse(400, {}, `You can reapply after ${daysLeft} day(s).`));
        }
    }
...
const resetPassword = asyncHandler(async (req, res) => {
    ...
    const resetPassToken = jwt.verify(otpToken, process.env
...
export { registerUser, loginUser, logoutUser, refreshAccessToken, getCurrentUser, updateProfile, resetPassword };
