...
export const verifyOtp = async (email, otp, purpose) => {
    ...
    const isCorrect = await bcrypt.compare(cleanOtp, hashedOtp);
    if (isCorrect) {
...
export const sendOTP = asyncHandler(async (req, res) => {
    ...
    if (purpose === "register") {
        await emailService.sendRegistrationOTP(cleanEmail, user?.name || "User", otp);
    }
    if (purpose === "login") {
...
export const checkOtp = asyncHandler(async (req, res) => {
    ...
    if (!isCorrect) {
        throw new ApiError(400, "OTP incorrect or expired");
    }
    /* REGISTER FLOW */
    if (purpose === "register") {
        const token = jwt.sign({ email: cleanEmail }, process.env.REGISTER_TOKEN_SECRET, {
    ...
    return res.status(200).json(new ApiResponse(200, { isCorrect: true }, "OTP verified successfully"));
});
