...
export const addToCart = asyncHandler(async (req, res) => {
    ...
    if (!cart) {
        ...
        const existingItem = cart.cards.find((item) => item.cardId.toString() === cardId);
        if (existingItem) {
...
export const removeCartCard = asyncHandler(async (req, res) => {
    ...
    const updatedCart = await Cart.findOneAndUpdate({ userId: req.user._id }, { $pull: { cards: { cardId } } }, { new: true }).populate("cards.cardId", "name price images");
    if (!updatedCart) {
...
export const totalCartAmount = asyncHandler(async (req, res) => {
    ...
    const deliveryThreshold = user.wholesalerStatus === "approved"
        ? 2000
        : 200;
    const deliveryCharge = taxableAmount >= deliveryThreshold ? 0 : 40;
...
export const updateCartCardQuantity = asyncHandler(async (req, res) => {
    ...
    const cart = await Cart.findOneAndUpdate({
        userId: req.user._id,
        "cards.cardId": cardId,
    }, { $set: { "cards.$.quantity": quantity } }, { new: true }).populate("cards.cardId", "name price images");
    if (!cart) {
