...
const createOrder = async (req, res) => {
    try {
        const { amount, currency = "INR", receipt, notes = {}, method = "all" } = req.body;
        if (!amount || amount <= 0) {
...
const verifyPayment = async (req, res) => {
    try {
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature, razorpay_vpa } = req.body;
...
const generateUpiQrCode = async (req, res) => {
    try {
        const { amount, name = "The Nimantran", description = "Payment for order", upi_id = process.env.MERCHANT_UPI_ID || "" } = req.body;
        if (!amount || amount <= 0) {
            return res.status(400).json({
                success: false,
                error: "Valid amount is required"
...
export { createOrder, verifyPayment, generateUpiQrCode, checkUpiPaymentStatus, refundPayment };
