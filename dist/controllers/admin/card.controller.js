...
export const createCard = asyncHandler(async (req, res) => {
    const { name, categories, price, quantityAvailable, specifications, isPopular, isTrending, primaryImageKey, secondaryImageKey, discount, wholesalePrice, wholesaleDiscount, isAvailableForWholesale, quantityPerBundleCustomer, quantityPerBundleWholesale, description } = req.body;
...
export const updateCard = asyncHandler(async (req, res) => {
    ...
    const { name, categories, price, quantityAvailable, discount, wholesalePrice, wholesaleDiscount, rating, description, reviewsCount, isAvailableForWholesale, isPopular, isTrending, specifications, primaryImageKey, quantityPerBundleCustomer, quantityPerBundleWholesale, secondaryImageKey } = req.body;
    if (!name || !categories || price === undefined || quantityAvailable === undefined) {
    ...
    const updateData = {
        ...
        quantityPerBundleCustomer: quantityPerBundleCustomer !== undefined
            ? Number(quantityPerBundleCustomer)
            : card.quantityPerBundleCustomer,
        quantityPerBundleWholesale: quantityPerBundleWholesale !== undefined
...
export const deleteCard = asyncHandler(async (req, res) => {
    ...
    if (!card)
        throw new ApiError(404, "Card not found");
    await Cart.updateMany({ "cards.cardId": card._id }, { $pull: { cards: { cardId: card._id } } });
    try {
        if (card.images?.primaryImageKey) {
            await deleteFromS3(card.images.primaryImageKey);
        }
    ...
    res.status(200).json(new ApiResponse(200, null, "Card deleted and removed from all carts"));
});
