...
export const verifyProductStock = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
...
export const verifyDiscountEligibility = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    if (!product.discount || product.discount <= 0) {
      res.status(400).json({
        success: false,
        message:
          "Product has no discount available.",
      })
      return
    }
...
export const verifyReviewInput = (
  req: Request,
  res: Response,
  next: NextFunction
): void => {
